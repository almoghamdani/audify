...
int rt_callback(void *outputBuffer, void *inputBuffer, unsigned int nFrames,
  ...
  if (wrap->_outputChannels) {
    ...
    if (!wrap->_outputData.empty()) {
      ...
      wrap->applyVolume(wrap->_outputData.front().get(), outputBuffer,
  ...
  if (wrap->_inputChannels) {
    ...
    if (wrap->_inputTsfn != nullptr) {
      // Copy the input data
      memcpy(inputData.get(), inputBuffer,
      ...
      wrap->_inputTsfn.NonBlockingCall(
          [wrap, inputData](Napi::Env env, Napi::Function callback) {
            callback.Call({Napi::Buffer<int8_t>::Copy(
                env, inputData.get(),
                wrap->_frameSize * wrap->_inputChannels * wrap->_sampleSize)});
          });
    }
...
Napi::Object RtAudioWrap::Init(Napi::Env env, Napi::Object exports) {
  ...
  Napi::Function ctor_func = DefineClass(
      ...
      {InstanceMethod("openStream", &RtAudioWrap::openStream),
       ...
       InstanceMethod("getDefaultInputDevice",
                      &RtAudioWrap::getDefaultInputDevice),
       InstanceMethod("getDefaultOutputDevice",
                      &RtAudioWrap::getDefaultOutputDevice),
       InstanceMethod("setInputCallback", &RtAudioWrap::setInputCallback),
       InstanceMethod("setFrameOutputCallback",
                      &RtAudioWrap::setFrameOutputCallback),
       InstanceAccessor("streamTime", &RtAudioWrap::getStreamTime,
                        &RtAudioWrap::setStreamTime),
       InstanceAccessor("outputVolume", &RtAudioWrap::getOutputVolume,
                        &RtAudioWrap::setOutputVolume)});
...
RtAudioWrap::RtAudioWrap(const Napi::CallbackInfo &info)
  ...
  RtAudio::Api api = info.Length() == 0
                         ...
                         : (RtAudio::Api)(int)info[0].As<Napi::Number>();
...
Napi::Value RtAudioWrap::getDevices(const Napi::CallbackInfo &info) {
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    devicesArray[i] =
        RtAudioConverter::ConvertDeviceInfo(info.Env(), devices[i]);
  }
...
Napi::Value RtAudioWrap::openStream(const Napi::CallbackInfo &info) {
  ...
  if (!errorCallback.IsEmpty()) {
    errorTsfn = Napi::ThreadSafeFunction::New(
        info.Env(), errorCallback, "errorCallback", 0, 1, [this](Napi::Env) {});
  }
  ...
  if (!inputCallback.IsEmpty() && !info[1].IsNull() && !info[1].IsUndefined()) {
    ...
    _inputTsfn = Napi::ThreadSafeFunction::New(
        info.Env(), inputCallback, "inputCallback", 0, 1, [this](Napi::Env) {});
  }
...
Napi::Value RtAudioWrap::getApi(const Napi::CallbackInfo &info) {
  return Napi::String::New(
      info.Env(), _rtAudio->getApiDisplayName(_rtAudio->getCurrentApi()));
}
...
void RtAudioWrap::applyVolume(void *src, void *dst, unsigned int amount) {
  ...
  for (unsigned int i = 0; i < amount; i++) {
    switch (_format) {
      case RTAUDIO_SINT8:
        *((int8_t *)dst + i) =
            (int8_t)(*((int8_t *)src + i) * _outputMultiplier);
        break;

      case RTAUDIO_SINT16:
        *((int16_t *)dst + i) =
            (int16_t)(*((int16_t *)src + i) * _outputMultiplier);
        break;

      case RTAUDIO_SINT32:
        *((int32_t *)dst + i) =
            (int32_t)(*((int32_t *)src + i) * _outputMultiplier);
        break;
      ...
      case RTAUDIO_FLOAT64:
        *((double *)dst + i) =
            (double)(*((double *)src + i) * _outputMultiplier);
        break;
    }
  }
}
